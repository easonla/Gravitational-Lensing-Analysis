...
class LensProperties:
    def __init__(self,potential,kappa):
        ...
        self._alpha = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
        self._alpha_x = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
        self._alpha_y = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
        self._gamma = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
        self._gamma1 = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
        self._gamma2 = Array2d(potential.xmin,potential.xmax,potential.nx,potential.ymin,potential.ymax,potential.ny)
    ...
    def _calc_div(self):
        ...
        gamma1 = np.zeros([n,n])
        gamma2 = np.zeros([n,n])
        gamma1[:,:]=0.5*((k[2:,1:-1]-2.*k[1:-1,1:-1]+k[0:-2,1:-1])/domi**2 - (k[1:-1,2:]-2.*k[1:-1,1:-1]+k[1:-1,0:-2])/domi**2)
...
def findbestfit(pf,w,theta=0,psi=0,phi=0,ngrid=256,listname='B09'):
    try:
        ...
        phi = xopt[0][2]
        chi = xopt[1]
        print "finish best minimize chi_sq, simtime = {:.3f} dx={:.3f} dy={:.3f} phi = {:.3f} theta = {:.3f} psi = {:.3f} fom = {:.5f}".format(simtime,dx,dy,phi,theta,psi,chi)
...
def chi_weak(x0,Lens,sheardata,taka_average=False):
    chi=0
    sigma = 1.
    sigma_eps_s = 0.2
    sigma_eps_er = 0.1
